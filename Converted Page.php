 ...
 * - $dbCol
 *
 * GAME CONFIG (MD3 — Pick 5 + Star Ball):
 * - game_id    = 'MD3'
 * - main balls = 5 (columns: first, second, third, fourth, fifth)
 * - main range = 01..39
 * - bonus ball = 1 Star Ball (column: sixth), range 01..39
 * - draw history: all balls searched across all six columns (game rule)
 */
...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    ...
    $doc->setMetaData('description', 'Explore verified ' . $gName . ' (' . $stateName . ') draw results, frequency analysis, hot and cold numbers, Star Ball history, and recency data. Analytical lottery intelligence powered by LottoExpert.');
}
...
[$mainCounts, $mainLastSeenIndex] = leInitRange($mainRangeMin, $mainRangeMax);
[$starCounts, $starLastSeenIndex] = leInitRange($bonusRangeMin, $bonusRangeMax);
 ...
 * --------------------------------------------------------------------------
 */
$mainChartLabels   = leBuildNaturalLabels($mainRangeMin, $mainRangeMax);
$mainChartValues   = [];
$mainRecencyValues = [];
...
$starChartLabels = leBuildNaturalLabels($bonusRangeMin, $bonusRangeMax);
$starChartValues = [];
...
[$counts50,  ] = leInitRange($mainRangeMin, $mainRangeMax);
[$counts300, ] = leInitRange($mainRangeMin, $mainRangeMax);
 ...
 * MD3 rule: history searched across all six columns for every ball
 * --------------------------------------------------------------------------
 */
$drawHistoryRows = [];
...
if ($drawDate !== '') {
    foreach ($latestMainBalls as $ball) {
        $prevDate = leGetPreviousOccurrenceDate($db, (string) $dbCol, $gameId, $drawDate, $ball, false, true);
    ...
    if ($hasBonusBall && $pb !== '') {
        $prevStarDate = leGetPreviousOccurrenceDate($db, (string) $dbCol, $gameId, $drawDate, $pb, false, true);
        $starDrawsAgo = leGetDrawingsSinceDate($db, (string) $dbCol, $gameId, $prevStarDate, $drawDate);

        $drawHistoryRows[] = [
            'label'    => $bonusLabel . ' (' . lePad2($pb) . ')',
 ...
 * --------------------------------------------------------------------------
 */
$jsonLdGameName  = isset($gName)     ? (string) $gName     : '';
$jsonLdStateName = isset($stateName) ? (string) $stateName : '';
$jsonLdPageName  = ($jsonLdStateName !== '' && $jsonLdGameName !== '')
    ? ($jsonLdStateName . ' ' . $jsonLdGameName . ' Results Intelligence')
    : 'Lottery Results Intelligence';
$jsonLdPageDesc  = ($jsonLdStateName !== '' && $jsonLdGameName !== '')
    ? ('Verified draw results, number frequency analysis, recency data, and Star Ball history for ' . $jsonLdGameName . ' (' . $jsonLdStateName . ') lottery.')
    : 'Verified lottery draw results, frequency analysis, and historical patterns.';
...
$jsonLdPayload = json_encode([
    ...
    '@graph'   => [
        [
            '@type'       => 'WebPage',
            'name'        => $jsonLdPageName,
            'description' => $jsonLdPageDesc,
            'url'         => $canonicalNoQuery,
        ],
        [
            '@type'       => 'Dataset',
            'name'        => ($jsonLdGameName !== '' ? $jsonLdGameName : 'Lottery') . ' Frequency Analysis',
            'description' => $jsonLdPageDesc,
            'url'         => $canonicalNoQuery,
        ],
    ],
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-ball-row" aria-label="Latest drawn numbers">
            ...
            <?php if ($hasBonusBall && $pb !== '') : ?>
              <span class="skai-ball-gap" aria-hidden="true"></span>
              <span class="skai-ball skai-ball--bonus" title="<?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars(lePad2($pb), ENT_QUOTES, 'UTF-8'); ?></span>
            <?php endif; ?>
          </div>

          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=MD3">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=MD3">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=MD3">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=MD3">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/all-lottery-heatmaps?gameId=MD3">Heatmap Analysis</a>
          </div>
        </div>
  ...
  <section class="skai-strip" aria-label="Key takeaways">
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        ...
        <div class="skai-stat-note">Main-number and <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> draw windows currently loaded for this page view.</div>
      </div>
    </article>
  </section>
    ...
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Start with a clear high-level view. This layer is designed for fast orientation: which numbers are most active, which are quiet, and how the current draw relates to recent history. Main-number pool covers values 01&ndash;<?php echo (int) $mainRangeMax; ?>; <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> is tracked separately.
        </p>
      </div>
    </div>
  ...
  <section id="recency-deep-dive" class="skai-section" aria-labelledby="recency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          This section makes the current draw easier to interpret. It shows how recently each drawn number was previously seen and how the perspective shifts when you compare a short recent window with a broader historical one. The <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> history is shown separately where applicable.
        </p>
      </div>
    </div>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The first panel shows the complete main-number distribution across all values 01&ndash;<?php echo (int) $mainRangeMax; ?>. The second panel shows the complete <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> distribution for the selected bonus window. The third shows how far each main number sits from its most recent appearance.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;<?php echo (int) $mainRangeMax; ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
        ...
        <div class="skai-grid">
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--radiant">
              ...
              <span class="skai-card-sub">All values 01&ndash;<?php echo (int) $bonusRangeMax; ?> across the last <?php echo (int) $nodCurrentStar; ?> drawings</span>
            </div>
            <div class="skai-card-body">
              <div class="skai-chart-shell">
                <div class="skai-chart-frame skai-chart-frame--bonus">
                  <canvas id="starChart" aria-label="<?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> distribution chart" role="img"></canvas>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. Main numbers and <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> values are drawn from the same pool (01&ndash;<?php echo (int) $mainRangeMax; ?>) but tracked independently by position.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the tables for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset. The main-number table covers values 01&ndash;<?php echo (int) $mainRangeMax; ?>; the <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> table covers values 01&ndash;<?php echo (int) $bonusRangeMax; ?> from the bonus column.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            Main numbers table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;<?php echo (int) $mainRangeMax; ?></span>
          </div>
          ...
          <div class="skai-table-wrap">
            <table id="skai-main-table" class="skai-table" aria-label="Main numbers frequency table">
              ...
              <tbody>
                <?php for ($i = $mainRangeMin; $i <= $mainRangeMax; $i++) : ?>
                  <?php
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;<?php echo (int) $bonusRangeMax; ?></span>
          </div>

          <div class="skai-table-wrap">
            <table id="skai-star-table" class="skai-table" aria-label="<?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> frequency table">
              <thead>
                <tr>
                  <th>Number</th>
                  <th>Drawn Times</th>
              ...
              <tbody>
                <?php for ($i = $bonusRangeMin; $i <= $bonusRangeMax; $i++) : ?>
                  <?php
                  $number      = ($i < 10) ? '0' . $i : (string) $i;
                  ...
                  <tr>
                    ...
                    <td>
                      <input
                        ...
                        type="checkbox"
                        value="<?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"
                        aria-label="Track <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> number <?php echo htmlspecialchars($number, ENT_QUOTES, 'UTF-8'); ?>"
                      >
                    </td>
                  </tr>
                <?php endfor; ?>
          ...
          <div class="skai-note">
            Tracking is local to this page view. It is intended as a lightweight comparison aid while you move between the overview, tables, and advanced SKAI tools. The <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> frequency is counted only from the bonus column, separate from main-number totals.
          </div>
        </div>
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=MD3">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=MD3">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=MD3">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?game_id=MD3">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/all-lottery-heatmaps?gameId=MD3">Heatmap Analysis</a>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. Main-ball counts are derived strictly from columns one through five; <?php echo htmlspecialchars($bonusLabel, ENT_QUOTES, 'UTF-8'); ?> counts are derived strictly from the sixth column. The purpose of this page is to make the recent behavior of <?php echo htmlspecialchars((string) $gName, ENT_QUOTES, 'UTF-8'); ?> easier to understand, compare, and carry into deeper SKAI analysis.
      </div>
    </div>
  </section>
...
(function () {
  ...
  function safeGetContext(id) {
    ...
    if (!el) { return null; }
    if (chartInstances[id]) { return null; }
  ...
  function renderCharts() {
    ...
    ctx = safeGetContext('topActiveChart');
    if (ctx && chartData.topActiveLabels && chartData.topActiveLabels.length) {
      chartInstances['topActiveChart'] = new Chart(ctx, {
        type: 'bar',
        data: {
    ...
    ctx = safeGetContext('quietChart');
    if (ctx && chartData.quietLabels && chartData.quietLabels.length) {
      chartInstances['quietChart'] = new Chart(ctx, {
        type: 'bar',
        data: {
    ...
    ctx = safeGetContext('fullMainChart');
    if (ctx && chartData.mainLabels && chartData.mainLabels.length) {
      chartInstances['fullMainChart'] = new Chart(ctx, {
        type: 'bar',
        data: {
    ...
    ctx = safeGetContext('starChart');
    if (ctx && chartData.starLabels && chartData.starLabels.length) {
      chartInstances['starChart'] = new Chart(ctx, {
        type: 'bar',
        data: {
    ...
    ctx = safeGetContext('recencyChart');
    if (ctx && chartData.mainLabels && chartData.mainLabels.length) {
      chartInstances['recencyChart'] = new Chart(ctx, {
        type: 'bar',
        data: {
          labels: chartData.mainLabels,
          datasets: [{
